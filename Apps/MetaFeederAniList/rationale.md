# MetaFeeder · AniList — Rationale

## What deviation / exception is being requested

`metafeederanilist-feeder` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  `/DATA/AppData/metafeederanilist/`, declared in `x-compose-app.folders` and owned `$PUID:$PGID`.
...
The feeder holds no user media. Its entire state lives under `/DATA/AppData/metafeederanilist/`, which
