# MetaPlugin · Full Hash — Rationale

## What deviation / exception is being requested

`metapluginfullhash-plugin` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  Its only bind is its own cache under `/DATA/AppData/metapluginfullhash/`, declared in
...
`/DATA/AppData/metapluginfullhash/`, which is the unit Maison archives on uninstall.
