# MetaPlugin · File Info — Rationale

## What deviation / exception is being requested

`metapluginfileinfo-plugin` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  Its only bind is its own cache under `/DATA/AppData/metapluginfileinfo/`, declared in
...
`/DATA/AppData/metapluginfileinfo/`, which is the unit Maison archives on uninstall.
