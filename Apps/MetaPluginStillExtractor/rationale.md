# MetaPlugin · Still Extractor — Rationale

## What deviation / exception is being requested

`metapluginstillextractor-plugin` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  `/DATA/Media` or `/DATA/Gallery`. It binds no host path at all. It reads the files it works on over
...
The plugin holds no user media and no user account. It is stateless: there is nothing to back up, and
a reinstall loses nothing.
