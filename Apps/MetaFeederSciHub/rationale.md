# MetaFeeder · Sci-Hub — Rationale

## What deviation / exception is being requested

`metafeederscihub-feeder` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  `/DATA/AppData/metafeederscihub/`, declared in `x-compose-app.folders` and owned `$PUID:$PGID`.
...
The feeder holds no user media. Its entire state lives under `/DATA/AppData/metafeederscihub/`, which
