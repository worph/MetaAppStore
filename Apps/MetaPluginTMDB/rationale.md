# MetaPlugin · TMDB — Rationale

## What deviation / exception is being requested

`metaplugintmdb-plugin` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  Its only bind is its own cache under `/DATA/AppData/metaplugintmdb/`, declared in
...
`/DATA/AppData/metaplugintmdb/`, which is the unit Maison archives on uninstall.
