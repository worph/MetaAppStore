# MetaFeeder · Suwayomi — Rationale

## What deviation / exception is being requested

`metafeedersuwayomi-feeder` runs `user: 0:0`.

## Why it is necessary

Carried over unchanged from the listing this app was split out of (MetaFeeder · Comic), so the
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  `/DATA/AppData/metafeedersuwayomi/`, declared in `x-compose-app.folders` and owned `$PUID:$PGID`.
...
The feeder holds no user media. Its entire state lives under `/DATA/AppData/metafeedersuwayomi/`, which
