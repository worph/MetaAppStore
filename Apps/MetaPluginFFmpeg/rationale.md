# MetaPlugin · FFmpeg — Rationale

## What deviation / exception is being requested

`metapluginffmpeg-plugin` runs `user: 0:0`.
...
- The container mounts **no user directories** — not `/DATA/Documents`, `/DATA/Downloads`,
  ...
  Its only bind is its own cache under `/DATA/AppData/metapluginffmpeg/`, declared in
...
`/DATA/AppData/metapluginffmpeg/`, which is the unit Maison archives on uninstall.
